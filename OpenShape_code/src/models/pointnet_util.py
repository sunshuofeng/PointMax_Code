...
from .PointNeXt.openpoints.models.layers.group import QueryAndGroup
from .PointNeXt.openpoints.models.backbone.pointmax_utils import *
...
def sample_and_group(npoint, xyz, points,group, returnfps=False):
    """
    Input:
        npoint:
        radius:
    ...
    grouped_xyz, grouped_points = group(new_xyz.contiguous(), xyz.contiguous(), points.permute(0,2,1).contiguous())
...
class Task_Branch(nn.Module):
    def __init__(self,  in_channel, mlp):
        ...
        out_channel=mlp[-1]
        self.fc=nn.Sequential(nn.Linear(out_channel,out_channel*2),nn.LayerNorm(out_channel*2),nn.GELU(),nn.Linear(out_channel*2,1280))
    ...
    def forward(self, new_points,label):
        ...
        loss=(1-F.cosine_similarity(cls_predict,label)).mean()
        return score,loss


class PointNetSetAbstraction_PointMax(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all,pointmax=False):
        super(PointNetSetAbstraction_PointMax, self).__init__()
        self.npoint = npoint
        self.radius = radius
        ...
        self.group=QueryAndGroup(radius,nsample)
        self.group_all = group_all
        self.pointmax=pointmax
        if pointmax:
            self.tob=Task_Branch(in_channel,mlp=mlp)
    ...
    def forward(self, xyz, points,label):
        ...
        if self.group_all:
            ...
            new_xyz, new_points = sample_and_group(self.npoint, xyz, points,self.group)
        ...
        B,C,N,K=new_points.shape
        if self.pointmax:
            group_xyz=new_points[:,:3]
            group_point=new_points[:,3:]
            global_f=torch.max(group_point,dim=-1).values
            pointmax_predict=self.pointmax_predict(group_xyz,new_xyz,global_f)
            if self.training:
            ...
            pointmax_index=torch.argsort(pointmax_predict,dim=-1,descending=True)[:,:,:int(K*0.5)]
