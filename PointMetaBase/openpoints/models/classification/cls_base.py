...
class BaseCls(nn.Module):
    ...
    def forward(self, data):
        global_feat,loss = self.encoder.forward_cls_feat(data)
        if self.training:
            return self.prediction(global_feat),loss
        else:
    ...
    def get_logits_loss(self, data, gt):
        logits,loss = self.forward(data)
        return logits, self.criterion(logits, gt.long())+0.1*loss
