...
class Task_Branch(nn.Module):
    def __init__(self,  in_channel, mlp):
        ...
        out_channel=mlp[-1]
        self.fc=nn.Sequential(nn.Linear(out_channel,out_channel//4),nn.BatchNorm1d(out_channel//4),nn.ReLU(),nn.Dropout(0.25),nn.Linear(out_channel//4,15))
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if pointmax:
            ...
            self.tob=Task_Branch(in_channels,mlp=channels)
            self.pointmax_predict=ConditionMLP(in_channels,32)
        if not is_head:
            if self.all_aggr:
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if not self.all_aggr:
                ...
                new_f=torch.gather(f,-1,idx.unsqueeze(1).expand(-1,f.shape[1],-1))
            else:
                new_p = p
                new_f=f
            ...
            if self.pointmax:
                ...
                # global_f=torch.max(group_f,dim=-1).values
                pointmax_predict=self.pointmax_predict(group_xyz,new_p,new_f)
                if self.training:
                    pointmax_score,pointmax_loss=self.tob(fj,label)
                    loss=F.mse_loss(pointmax_predict,pointmax_score.clone().detach())
                    loss=loss+pointmax_loss
                pointmax_index=torch.argsort(pointmax_predict,dim=-1,descending=True)[:,:,:int(K*self.K)]
                fj=fj.gather(dim=3,index=pointmax_index.unsqueeze(1).repeat(1,C,1,1))
...
class PointNextEncoder_PointMax(nn.Module):
    ...
    def __init__(self,
        ...
        self.pointmax_layers=kwargs.get('pointmax_layers')
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                ...
                is_head=i == 0 and strides[i] == 1,pointmax=self.pointmax_layers[i]
            ))
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False,pointmax=False):
        layers = []
        ...
        layers.append(SetAbstraction(self.in_channels, channels,
                                     ...
                                     is_head=is_head, use_res=self.sa_use_res,pointmax=pointmax, **self.aggr_args 
                                     ))
    ...
    def forward_cls_feat(self, p0, f0=None,target=None):
        if self.training:
            target=p0['y']
        if hasattr(p0, 'keys'):
        ...
        for i in range(0, len(self.encoder)):
            p0, f0,target,loss = self.encoder[i]([p0, f0,target])
            all_loss=all_loss+loss
        
        return f0.squeeze(-1),all_loss
